...
def immuniser(l, l2, p):
    """
    l: infectés; l2: immunisés précédents
    """
    drop = 0
    for i in range(len(l)):
...
def deces(l, l2, l3, p):
    """
    l: infectés; l2: décès précédents; l3: immunisés
    """
...
def vague_seuil_px_opti2():
    ...
    if data['coord_immunises']:
        fig.add_trace(go.Scatter(x=np.array(data['coord_immunises'])[:, 0], y=np.array(data['coord_immunises'])[:, 1],
    ...
    fig.add_trace(go.Pie(values=[len(data['coord_sains']), len(data['coord_infectes']), len(data['coord_immunises']),

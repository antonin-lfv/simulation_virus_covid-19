...
def vague_seuil_px_opti():
    ...
    courbes = dict(courbe_sains = [],courbe_infectes = [],courbe_immunises = [],courbe_deces = [],courbe_removed = [])
    ...
    courbes['courbe_immunises'].append(0)
    courbes['courbe_deces'].append(0)
    courbes['courbe_removed'].append(0)

    # 1er vague

    df_sans1erinfecte = df[(df['x'] != df['x'][numero_infecte_1]) & (df['y'] != df['y'][numero_infecte_1])]
    coord = dict(coord_infectes=[],coord_sains=[],coord_immunises=[],coord_deces=[])
    for k in range(taille_pop):
    ...
    i = 1
    while len(coord['coord_infectes']) > 0.05 * taille_pop or len(courbes['courbe_sains']) < 15:
        non_sains = []
        ...
        courbes['courbe_sains'].append(len(coord['coord_sains']))
        courbes['courbe_infectes'].append(len(coord['coord_infectes']))
        courbes['courbe_immunises'].append(len(coord['coord_immunises']))
        courbes['courbe_deces'].append(len(coord['coord_deces']))
        courbes['courbe_removed'].append(len(coord['coord_immunises']) + len(coord['coord_deces']))
        i += 1  # vague suivante
    if coord['coord_sains'] != []:
    ...
    labels = ["sains", "infectés", "immunisés", "décédés"]
    fig.add_trace(go.Pie(values=[len(coord['coord_sains']), len(coord['coord_infectes']), len(coord['coord_immunises']), len(coord['coord_deces'])], labels=labels, sort=False), 1, 2)

    x_courbe = list(np.arange(0, len(courbes['courbe_sains'])))
    fig.add_trace(go.Scatter(x=x_courbe, y=courbes['courbe_sains'], marker=dict(color='#636EFA'), showlegend=False, name="sains",yaxis="y", ), 2, 1)
    fig.add_trace(go.Scatter(x=x_courbe, y=courbes['courbe_infectes'], marker=dict(color='#EF553B'), showlegend=False, name="infectés",yaxis="y2", ), 2, 1)
    fig.add_trace(go.Scatter(x=x_courbe, y=courbes['courbe_immunises'], marker=dict(color='#00CC96'), showlegend=False, name="immunisés",yaxis="y3", ), 2, 1)
    fig.add_trace(go.Scatter(x=x_courbe, y=courbes['courbe_deces'], marker=dict(color='#AB63FA'), showlegend=False, name="décédés",yaxis="y4", ), 2, 1)
    fig.add_trace(go.Scatter(x=x_courbe, y=courbes['courbe_removed'], marker=dict(color='#000000'), showlegend=False, name="removed",yaxis="y5", ), 2, 1)
    fig.update_xaxes(title_text="jours", row=2, col=1)
    fig.update_yaxes(title_text="nombre d'individus", row=2, col=1)
    fig.add_annotation(text="Maximum d'infectés", x=courbes['courbe_infectes'].index(max(courbes['courbe_infectes'])),# ajouter un texte avec une flèche
                       y=max(courbes['courbe_infectes']) + 0.03 * taille_pop, arrowhead=1, showarrow=True, row=2, col=1)
    fig.update_traces(
        hoverinfo="name+x+y",
        line={"width": 1.2},
        marker={"size": 6},
